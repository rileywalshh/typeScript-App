//Imports from the Artwork and Artists interfaces.
...
const otherArtists = document.getElementById('artists') as HTMLDivElement | null;
const featuredArtists = document.getElementById('featuredArtists') as HTMLDivElement | null;
const addArtistButton = document.getElementById('addArtistButton') as HTMLButtonElement | null;
const displayFeaturedArtistsButton = document.getElementById('displayFeaturedButton') as HTMLButtonElement | null;
const displayAllArtistsButton = document.getElementById('displayAllButton') as HTMLButtonElement | null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkRequiredFields = (): boolean => {
        ...
        const requiredFields = document.querySelectorAll<HTMLInputElement | HTMLSelectElement>('#addArtist input[required], #addArtist select[required]');
    ...
    if (searchButton) {
        searchButton.addEventListener('click', () => {
            const searchId = (document.getElementById('searchArtistId') as HTMLInputElement).value;
    ...
    if (displayAllArtistsButton){
        displayAllArtistsButton?.addEventListener('click', () => {
        displayAllArtists();
        });
    }
});
...
function addArtistToList(newArtist: Omit<Artist, 'id'>): void {
    ...
    const artistWithId = { ...newArtist, id: artistId.toString() };
...
function featureArtistById(artistId: string): void {
...
function generateArtistsTable(): void {
    ...
    if (artistsDiv) {
        ...
        artists.filter((artist: Artist) =>  !artist.isFeatured).forEach((artist: Artist) => {
            ...
            const artistData: string[] = [
                artist.id,
                artist.name,
                artist.dob.toLocaleDateString(),
                artist.gender,
                artist.contactInfo,
                artist.artworks.map((artwork: Artwork) => `${artwork.type}, ${artwork.exhibitionDate.toLocaleDateString()}, Notes: ${artwork.specialNotes || 'None'}`).join('; '),
            ...
            artistData.forEach((text: String) => {
    ...
    function generateFeaturedArtistsTable(): void {
...
function handleFormSubmit(event?: Event) {
    ...
    const artistGender = (document.getElementById('artistGender') as HTMLSelectElement).value;
    const artistContactInfo = (document.getElementById('artistEmail') as HTMLInputElement).value;
    const artworkType = (document.getElementById('artWorkType') as HTMLSelectElement).value;
    const exhibitionDate = (document.getElementById('exhibitionDate') as HTMLInputElement).value;
    const specialNotes = (document.getElementById('specialNotes') as HTMLTextAreaElement).value;
    ...
    const newArtist: Artist = {
        ...
        gender: artistGender as 'Female' | 'Male' | 'Unspecified' | 'Anonymous',
        ...
        artworks: [{
            type: artworkType as 'painting' | 'sculpture' | 'photograph' | 'video art' | 'digital art' | 'printmaking',
...
function deleteArtist(artistId: string): void {
